...
with open("/mnt/bioinf/archive0/eigen_thread/scop_data/non_redundant_list_family_level.txt") as non_redundant:
...
scop_list = {}
with open("/mnt/bioinf/archive0/eigen_thread/scop_data/pdb_scop_class.txt") as scop_list_file:
...
bench_membership = {}
with open('/mnt/bioinf/archive0/eigen_thread/scop_data/benchmark_family_members.txt') as benchfile:
...
for result_dir in glob.glob("/mnt/bioinf/archive0/eigen_thread/results/distance/t*"):
# for result_dir in glob.glob("/cs/research/bioinf/home1/green/dbuchan/archive0/eigen_thread/results/random/L*"):
    print(result_dir)
    # if result_dir != "/mnt/bioinf/archive0/eigen_thread/results/distance/c1":
    #     continue
    for file in glob.glob(result_dir+"/*.out"):
        ...
        if pdb_id in bench_membership:
            ...
            with open(file) as csvresult:
                ...
                for line in lines:
                    if line[6] in omit_from_results_set:
                        # print("SKIPPING")
                        pass
                    else:
                        try:
